...
module.exports = {
	...
	/*
	 * 获取排行榜前100名的数据， 成功返回json数据对象
	*/ 
	getListData: function(uid, callback){
    ...
    /*
     * uid: NativeManager.getUid(),
     * userInfo : {
     	  name: "",
     	  avatar: 123
     * }
     *
     * 返回：成功true + 我排名附近（包括我自己）的3个人的排行榜数据dataJson
    */    
    setScore: function(uid, userInfo, score, callback){
    	var xhr = new XMLHttpRequest();
    	xhr.onreadystatechange = function () {
            if (xhr.readyState == 4) {
                if((xhr.status >= 200 && xhr.status < 400)){
    ...
    getUserInfoParams: function(uid, info, topScore){
